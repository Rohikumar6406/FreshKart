  ...
  return (
    <header
      className={`bg-white fixed top-0 right-0 left-0 z-50 ${
        isScrolled ? "drop-shadow-[0_4px_25px_rgba(0,0,0,.3)]" : ""
      }`}
    >
      <nav className="max-w-[1400px] mx-auto px-10 md:h-[14vh] h-[12vh] flex justify-between items-center">
        <Link to="/" className="text-3xl font-bold">
          <span className="text-orange-500 uppercase">F</span>resh
          <span className="text-orange-500 uppercase">K</span>art
        </Link>
        ...
        {/* Mobile Menu */}
        <ul
          className={`shadow-xl flex flex-col gap-y-12 bg-orange-500/15 backdrop-blur-xl rounded-xl p-10 items-center gap-x-15 md:hidden absolute top-30 -left-full transform -translate-x-1/2 transition-all ${
            showMenu ? `left-1/2` : ""
          }`}
        >
          <li>
